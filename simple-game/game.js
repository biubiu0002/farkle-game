console.log('Farkle游戏已加载 - 版本 v2.6 - 初始Farkle检测 + 完善Farkle处理');
...
function startGame() {
  ...
  // 检查初始骰子是否是 Farkle
  if (isFarkle(rawDice)) {
    console.log('初始骰子Farkle！骰子:', rawDice);
    // 直接切换玩家
    const currentPlayer = gameState.players[gameState.currentPlayer]
    gameState.currentPlayer = (gameState.currentPlayer + 1) % 2
    gameState.gamePhase = 'selecting'
    // 生成新玩家的骰子
    const newPlayerDice = rollDice(6)
    gameState.rolledDice = newPlayerDice.map((value, idx) => ({ value, index: idx }))
    gameState.unheldDice = [...gameState.rolledDice]
    gameState.message = `${currentPlayer.name} 初始Farkle！轮到${gameState.players[gameState.currentPlayer].name}`
    console.log('初始Farkle，直接切换玩家到', gameState.currentPlayer);
  } else {
    gameState.gamePhase = 'selecting'
...
function rollDiceAction() {
  ...
  if (isFarkle(newDice)) {
    ...
    gameState.heldDice = []
    gameState.currentRoundScore = 0
    gameState.message = `${currentPlayer.name} Farkle了！本轮得分清空，轮到${gameState.players[gameState.currentPlayer].name}`
    console.log('Farkle，直接切换玩家到', gameState.currentPlayer);
  } else {
...
function rollAgain() {
  ...
  // 从操作堆(unheldDice)中筛选未选中的骰子，而不是从所有骰子中筛选
  ...
  console.log('将摇的骰子数量:', diceToRoll, '是否Hot Dice:', unselectedDice.length === 0);
  ...
  if (isFarkle(newDice)) {
    ...
    // 剩余骰子Farkle了，直接切换玩家，不显示Farkle状态
    const currentPlayer = gameState.players[gameState.currentPlayer]
    gameState.currentPlayer = (gameState.currentPlayer + 1) % 2
    gameState.gamePhase = 'selecting'
    // 生成新玩家的骰子
    ...
    // 检查是否Hot Dice（所有骰子都被使用）
    const isHotDiceTurn = unselectedDice.length === 0
    console.log('是否Hot Dice回合:', isHotDiceTurn);

    if (isHotDiceTurn) {
      // Hot Dice：清空已保留骰子，使用新的6个骰子
      gameState.rolledDice = rolledDiceWithIndex
      gameState.heldDice = []
      gameState.unheldDice = rolledDiceWithIndex
    } else {
